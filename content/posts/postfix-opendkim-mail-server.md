...
<figure>
  <img src="/images/postfix-mail-flow.png" alt="Self-hosted mail architecture: Local App → Postfix (null client) → OpenDKIM (DKIM signing) → SMTP Relay (TLS, port 587) → Recipient Inbox, with DNS records SPF, DKIM, DMARC, and PTR verified by the recipient server" width="880" height="420" loading="lazy" style="width:100%;height:auto;border-radius:0.5rem;border:1px solid var(--color-line)" />
